...
public class Host {
	...
	//Ahmad
	public double getTotalCPUUtilization(double currentTime) {
		double hostUtilization = 0;

		for (Vm vm : getVmList()) {
			hostUtilization += vm.getVmCPUUtilization(currentTime);
		}

		return hostUtilization;
	}

	public double getTotalRAMUtilization(double currentTime) {
		double hostUtilization = 0;

		for (Vm vm : getVmList()) {
			hostUtilization += vm.getVmRAMUtilization(currentTime);
		}
