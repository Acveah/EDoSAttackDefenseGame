...
public class Datacenter extends SimEntity {
	...
	private static double currentUtilizationRAM = 0;
...
//				break;
			...
			case CloudSimTags.Check_Average_RAM_Utilization_every_second:
				if(CloudSim.clock() <= 12 * 60){
					//没能实现。
					getAverageRAMUtilization1();
					setNewVmResultInf();
					send(getId(), 1, CloudSimTags.Check_Average_RAM_Utilization_every_second);
				}
				break;

//			case CloudSimTags.Check_Average_RAM_Utilization:
//				checkAverageRAMUtilizationForScalling();
//				if(CloudSim.clock() <= 12 * 60){
//					send(getId(), CloudSim.getScaleUpTimer(), CloudSimTags.Check_Average_RAM_Utilization);
//				}
//				break;
	...
	protected void getAverageRAMUtilization1() {
		double totalHostUtilization = 0;

		List<? extends Host> list = getVmAllocationPolicy().getHostList();
		int totalHostCount = list.size();
		// for each host...
		for (int i = 0; i < list.size(); i++) {
			Host host = list.get(i);
			double mem = (double) host.getRam();
			double used = (double) host.getRamProvisioner().getUsedRam();
			totalHostUtilization += used/mem;
		}
		double averageUtilization = totalHostUtilization/totalHostCount;
		currentUtilizationRAM = averageUtilization*100;
	}

	protected void getAverageRAMUtilization2() {
		//No Reason
		double averageUtilization = 0, totalHostUtilization = 0;
		int totalVmCount = 0;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			totalHostUtilization += host.getTotalRAMUtilization(CloudSim.clock());
			totalVmCount += host.getVmList().size();
		}
		...
		currentUtilizationRAM = averageUtilization;
		VMCount = totalVmCount;
	}
	...
	public void setNewVmResultInf() {
		if(CloudSim.clock() > 0) {
			double averageUtilization = currentUtilization;
			double averageUtilizationRAM = currentUtilizationRAM;
			...
			getAverageVmResultInformation().add(new SystemInformation((int) CloudSim.clock(), totalVmCount, averageUtilization,averageUtilizationRAM, rt, getTotalRequestsServed()));
