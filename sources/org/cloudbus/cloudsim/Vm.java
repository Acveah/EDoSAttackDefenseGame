...
public class Vm {
	...
	private double timeOfLastBusy = 0.0;

	private ServerInformation informatiom = new ServerInformation();
	...
	//Ahmad
	public double getVmCPUUtilization(double currentTime) {
		double utilization;

		if(getNextAvailableTime() > currentTime) {
			utilization = informatiom.getProcessingTime() + currentTime - getTimeOfLastBusy() - previosPT;
			previosPT = informatiom.getProcessingTime() + currentTime - getTimeOfLastBusy();
			//utilization = ((informatiom.getProcessingTime() + (currentTime - getTimeOfLastBusy())) / (currentTime - informatiom.getInitiationTime())) * 100;
		} else if((currentTime - informatiom.getInitiationTime()) > 0){
			utilization = informatiom.getProcessingTime() - previosPT;
			previosPT = informatiom.getProcessingTime();
			//utilization = ((informatiom.getProcessingTime()) / (currentTime - informatiom.getInitiationTime())) * 100;
		} else {
			utilization = 0;
		}
	...
	public double getVmRAMUtilization(double currentTime) {
		double utilization;
		double ram = getCurrentAllocatedRam();
		utilization = ram/getRam();
		//return utilization;
		return (utilization * 100);
	}
	...
	public void setNextAvailableTime(double time) {
	...
	public void setTimeOfLastBusy(double time) {
		timeOfLastBusy = time;
	}

	public double getTimeOfLastBusy() {
		return timeOfLastBusy;
	}
